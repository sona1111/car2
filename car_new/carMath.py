...
logr = logging.getLogger('car.carMath')

'''
speedtable takes an input dict from the parent class which defines the recommended speed based on a distance reading from the front sensor (later this will take all sensors into account) Should be passed around as opposed to re-initialized
'''
class SpeedTable(object):

    def __init__(self, speedDict):
        
        pass
        
    #perform the calculations and return a speed based on a reading. (reading is in cm)
    def get(speed):
